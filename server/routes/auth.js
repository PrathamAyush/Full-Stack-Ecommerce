...
router.post("/register", (req, res) => {
    ...
    User.findOne({ email: email })
        .then((userInDb) => {
            if (userInDb) {
                return res.status(502).json({ err: "User Already Exist" })
            }
            bcrypt.hash(password, 16)
                .then((hashPassword) => {
                    ...
                    newUser.save()
                        .then(() => {
                            console.log(newUser)
                            return res.status(201).json({ result: "user Registered success" })
                        }).catch((err) => {
            ...
            bcrypt.compare(password, userInDb.password)
                .then((userMatch) => {
                    if (userMatch) {
                        ...
                        const userInfo = { "fullName": userInDb.fullName, "email": userInDb.email }
                        res.status(200).json({ result: { user: userInfo, token: token } })
                    }
                    else {
                        return res.status(500).json("Invalid Credential")
                    }
                })
        })
})
